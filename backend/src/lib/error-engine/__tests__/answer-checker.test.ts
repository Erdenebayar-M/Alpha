...
describe('TT_2_3 via taskType', () => {
  it('exact match → isCorrect: true, empty arrays', () => {
    const r = checkAnswer('ном', 'ном', 'TT_2_3');
  ...
  it('mismatch → isCorrect: false', () => {
    expect(checkAnswer('ном', 'нум', 'TT_2_3').isCorrect).toBe(false);
  });

  it('case differs → isCorrect: false (case-sensitive)', () => {
    expect(checkAnswer('ном', 'Ном', 'TT_2_3').isCorrect).toBe(false);
  });

  it('whitespace is normalised before comparison', () => {
    expect(checkAnswer('ном', '  ном  ', 'TT_2_3').isCorrect).toBe(true);
  });

  it('NFC normalisation: composed and decomposed forms are equal', () => {
    expect(checkAnswer('э', 'э', 'TT_2_3').isCorrect).toBe(true);
  });

  it('collapses multiple internal spaces', () => {
    expect(checkAnswer('ном гэр', 'ном  гэр', 'TT_2_3').isCorrect).toBe(true);
  });
});

describe('TT_2_4 via taskType', () => {
  it('correct fill → isCorrect: true', () => {
    expect(checkAnswer('о', 'о', 'TT_2_4').isCorrect).toBe(true);
  });

  it('wrong fill → wrongChars entry', () => {
    const r = checkAnswer('о', 'у', 'TT_2_4');
  ...
  it("case-insensitive: 'а' matches 'А'", () => {
    expect(checkAnswer('а', 'А', 'TT_2_4').isCorrect).toBe(true);
  });
});

describe('TT_7_3 via taskType', () => {
  it('exact word-list → isCorrect: true', () => {
    expect(checkAnswer('ном гэр нар', 'ном гэр нар', 'TT_7_3').isCorrect).toBe(true);
  });

  it("'ном гр нар' vs 'ном гэр нар' → missingChars includes 'э'", () => {
    const r = checkAnswer('ном гэр нар', 'ном гр нар', 'TT_7_3');
    expect(r.isCorrect).toBe(false);
    expect(r.missingChars.some((c) => c.char === 'э')).toBe(true);
  });

  it("wrong word 'ном кэр нар' → wrongChars with г→к", () => {
    const r = checkAnswer('ном гэр нар', 'ном кэр нар', 'TT_7_3');
...
describe('TT_8_4 via taskType', () => {
  it('identical → isCorrect: true', () => {
    expect(checkAnswer('гэр', 'гэр', 'TT_8_4').isCorrect).toBe(true);
  });

  it('wrong char → isCorrect: false', () => {
    const r = checkAnswer('гэр', 'кэр', 'TT_8_4');
    expect(r.isCorrect).toBe(false);
