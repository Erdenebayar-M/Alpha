 ...
 *
 * The mascot is the play control: it is the ORto globe wearing its earbuds,
 * which components/brand/Mascot.tsx already is — that file is a port of the
 * same Figma component ("Sound idle component", node 1202:7512) this card
...
export default function AudioChoice({ task, position, total, onResult }: ExerciseProps<AudioTask>) {
  ...
  const media = (
    ...
    <div className="relative w-full [--mascot-w:clamp(180px,34vw,282px)]">
      <audio
        ref={audioRef}
        preload="none"
        src={task.audioSrc}
      ...
      <button
        ...
        className={`mx-auto block w-[var(--mascot-w)] rounded-card transition-transform duration-150 ease-press focus-ring ${
